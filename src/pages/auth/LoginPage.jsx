...
export const LoginPage = () => {
  ...
  const handleDemoAdminLogin = () => {
    const demoAdminUser = {
      id: 'demo_admin_60d5ec49f1b2c81128d54779',
      firstName: 'System',
      lastName: 'Administrator',
      email: 'admin@leadms.org',
      role: ROLES.ADMIN,
    };
    const demoToken = 'demo_admin_access_token_123';

    tokenStorage.setAccessToken(demoToken);
    tokenStorage.setRefreshToken(demoToken);
    tokenStorage.setUser(demoAdminUser);
    ...
    addToast({
      type: 'info',
      title: 'Admin Console Preview Mode',
      message: 'Logged in as System Administrator for Admin UI inspection.',
    });
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8 shadow-soft-lg backdrop-blur-md">
      ...
      <div className="mt-4 pt-4 border-t border-dashed border-slate-200 dark:border-slate-800">
        <Button
          type="button"
          variant="outline"
          className="w-full text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-900/50 hover:bg-purple-50 dark:hover:bg-purple-950/40 text-xs font-bold"
          leftIcon={<ShieldCheck className="w-4 h-4 text-purple-600" />}
          onClick={handleDemoAdminLogin}
        >
          Sign In as Admin (Console Preview)
        </Button>
      </div>
